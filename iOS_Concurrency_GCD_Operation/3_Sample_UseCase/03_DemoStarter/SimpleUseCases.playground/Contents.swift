...
func asyncAdd(runQueue:DispatchQueue, completionQueue:DispatchQueue, input:(Int,Int), completion:@escaping(Int,Error?)->()) {
    runQueue.async {
...
asyncAdd(runQueue: userQueue, completionQueue: defaultQueue, input: (1,2)) { (result, error) in
    print("Async task addition result : \(result)")
}
